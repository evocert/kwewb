...
(function () {
	...
	define(function () {
		...
		css = {
			...
			load: function (name, req, load,config) { //, config (not used)
				if(config.protocol=="ws://"){
					...
					url=window.location.protocol+"//"+"localhost"+":"+window.location.port+window.location.pathname.substring(0,window.location.pathname.lastIndexOf("/")+1)+url;
					window.ws.send(url).then(function(val){
 function addcss(css){

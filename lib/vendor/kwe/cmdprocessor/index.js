	...
	function CMDProcessor(){
		...
		*/
		...
		this.exec=function(args){
			//args=str2argv(args);
			args=arggen(args);
			if(typeof(this[args[0]])=="function")return this[args[0]].bind(this).apply(this,args.slice(1));
			if(typeof(this.modules[args[0]])=="function")return this.modules[args[0]].bind(this).apply(this,args.slice(1));
			...
			try{//basic usage
				...
				cmd.exec(["cd",this.fileManager.cwd].join(" "));
				...
				try{
					...
					require([modpath],function(mod){
						...
						if(typeof(mod)=="function"){
							...
							var t1=new Date();
							result="module loaded in "+(t1-t0)+" milliseconds";
						}else{
		...
		this.set=function(arg){
			if(arg==null){
				...
				cache[arg.split("=")[0]]=(
					[arg.split("=")[1]]
					.concat(Object.values(arguments).slice(1))
				).join(" ");
				else
					return "Invalid usage"
			}

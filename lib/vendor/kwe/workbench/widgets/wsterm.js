define([
	...
	function Terminal(args){
		...
		args.pollinginterval=typeof(args.pollinginterval)=="number"?args.pollinginterval:500;
		args.svcurl=typeof(args.svcurl)=="number"?args.svcurl:"www/kwewb/lib/vendor/kwe/srvcommand/index.js";
		...
		this.panel=jsPanel.create({
			...
			callback:function(panel){
				...
				panel.term=container.terminal(
					function(cmd,term){
						term.executing=true;
						//sendeval(term.socket,`cmd.exec(${JSON.stringify([cmd,< @ print(JSON.stringify(osutils.GOOS()=="linux"?" 2>&1":" 2>&1")); @ >].join(""))});`);
						sendeval(term.socket,'cmd.exec('+JSON.stringify(cmd)+'+" 2>&1");');
					}.bind(this),
					{
						...
						onInit:function(term){
							...
							function flush(){
								if(!term.executing){
									sendeval(term.socket,`try{print(cmd.flush());}catch(e){}`);
								}
							}
							function setupws(){
		...
		try{//basic usage
			this.cmd=typeof(this.cmd)=='undefined'?(function(){
				return new SrvCommand({
					"path":"/bin/bash",///*< @ print(JSON.stringify(osutils.GOOS()=="linux"?"/bin/bash":"cmd.exe")); @ >,*/
					"args":[],
					"timeout":10,
...
`
			...
			onclosed:function(panel){
				...
				//window.clearTimeout(panel.term.poller);
			}
		});
	};
